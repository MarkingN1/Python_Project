...
TITLE_FONT = pygame.font.SysFont("Caslon",100)
...
class Slider:
    ...
    def draw(self, screen):
        pygame.draw.rect(screen, TEAL, self.rect)
...
class ToggleSwitch:
    ...
    def draw(self, screen):
        pygame.draw.rect(screen, GREEN if self.state else BUTTON_HOVER_COLOR, self.rect)
        text_surf = FONT.render("On" if self.state else "Off", True, BLACK)
        text_rect = text_surf.get_rect(center=self.rect.center)
        screen.blit(text_surf, text_rect)
        label_text = FONT.render("Music", True, WHITE)
...
def draw_title(screen, text, y_position):
    title_surface = TITLE_FONT.render(text, True, PINK)
...
def gamemodes():
    ...
    snake = Button(300, 250, 200, 60,'Snake', run_snake)
